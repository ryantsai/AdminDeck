...
- Native context menu icons are app-owned SVG strings in `src/lib/nativeMenuIcons.ts`. Pass them as `iconSvg` through `src/lib/nativeContextMenu.ts`, which rasterizes the SVG to a small RGBA Tauri `Image` before opening the menu. Do not pass raw SVG paths directly to Tauri menu APIs and do not add one-off PNG icon files for simple menu commands.
