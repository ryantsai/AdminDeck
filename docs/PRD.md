...
- UI model: left Activity Rail with Workspace, Dashboard, App Launcher, File Explorer, and Settings entries, left-side connection manager/tree with root Connections and optional nested folders (visible inside the Workspace module), main module content area, right AI Assistant panel, and a bottom app-wide **Status Bar** that remains visible across modules and pages.
- Status Bar model: the left segment is module-owned; Workspace shows host CPU, RAM, and aggregate network usage, while Settings intentionally leaves this segment empty for now. The center segment is the universal notifications text area used by all modules for transient status and error notices.
