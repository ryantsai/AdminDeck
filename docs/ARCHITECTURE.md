...
The Dashboard module supports multiple local views. The first view is the default view; additional views are local UI state that group selected Dashboard widgets and reports without creating backend Sessions or saved Connections. Dashboard supplies the shared right AI Assistant panel with an active page-context summary for the current view, including selected widgets and report summaries. Keep that context separate from terminal selected-output context in the assistant request model.
